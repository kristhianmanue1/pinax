...
| H-2 | `skopos/project-manifest.yaml` commiteado tiene YAML malformado (colon sin comillas en "…ADR-014): Ollama…"); `build` fail-closed; el lint no cosecha aristas de skopos | `VERIFIED` | scripts/pinax.py build 2026-09-10; commit 3a3f37a | **RESUELTA (adjudicación del Mediador, cierre F0.1):** reparación publicada por el mantenedor de skopos en `def9bb95eb8600d3ff15316859138437df56963e` (origin/main verificado por `ls-remote`; bloque plegado `>-`, semántica byte-idéntica). El blob publicado parsea y valida contra pinax, y es idéntico al working tree: skopos entra al mapa machine-readable con sus 8 contratos y aristas cosechadas, sin dependencia de estado local |
...
| H-9 | `epistates/task-card` viaja con dos convenciones de id: `…/task-card/v1` (epistates publica, ektel consume) vs `…/task-card` + `version: v1` (pinax consume); el lint no los empareja | `VERIFIED` | manifiestos de epistates, ektel, pinax; lint 2026-09-10 | **RESUELTA (decisión del Mediador, cierre F0.1):** el productor (Epistates) estableció `contract_id = epistates/task-card/v1`; la versión forma parte integral del identificador y no existe campo `version` separado; la identidad se resuelve por igualdad exacta. Corregido sólo el `consume` de pinax (`project-manifest.yaml`): `epistates/task-card` + `version: v1` → `epistates/task-card/v1`. Sin aliases, sin canonicalización automática, sin cambios de schema. El grafo empareja Epistates → EKTEL y Epistates → Pinax por identidad exacta (lint: el hallazgo contractual desaparece) |
